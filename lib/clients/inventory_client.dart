...
class InventoryClient extends ChangeNotifier {
        ...
        (ResoniteDirectory value) {
          value.children.sort(
            (ResoniteDirectory a, ResoniteDirectory b) =>
                _sortMode.sortFunction(a.record as Record, b.record as Record, reverse: _sortReverse),
          );
          return value;
        },
      );
  ...
  Future<void> reloadCurrentDirectory() async {
    ...
    _currentDirectory = _getDirectory(dir.record as Record).then(
      (records) {
  ...
  Future<void> navigateTo(Record record) async {
    ...
    final childDir = dir.findChildByRecord(record as Record);
    if (childDir == null) {
    ...
    // the previous directory while also being able to display the error as a snack-bar.
    if (caughtError != null) {
      throw caughtError!;
    }
  }
